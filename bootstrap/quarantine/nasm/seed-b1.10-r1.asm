...
; Quarantaine temporaire autorisée pour le tracer B1.10 uniquement.
; Commande de matérialisation quarantainée :
; nasm -f bin bootstrap/quarantine/nasm/seed-b1.10-r1.asm -o bootstrap/quarantine/nasm/out/seed-b1.10-r1.efi
...
%define PE_TEXT_RAW             0x2000
%define PE_RDATA_RAW            0x600
...
SECTION .text start=PE_TEXT_FILE vstart=0x1000 align=16
text_start:
entry:
    push rbx
    push rbp
...
semantic_success:
    mov rax, [rbp + F_WORK]
    cmp qword [rax + 16], 0        ; Pas de payload data mesuré avant les directives.
    jne fail_e603
...
SECTION .rdata start=PE_RDATA_FILE vstart=0x3000 align=16
rdata_start:
loaded_image_guid:
...
success_b110:
    ...
    dw 'N','G','_','S','0','_','T','1','0','_','O','K',13,10,0
abi_kw:
    db 'abi'
efi_x64_kw:
    db 'efi_x64'
...
SECTION .data start=PE_DATA_FILE vstart=0x4000 align=8
data_start:
    dq 0
data_end:
    times PE_DATA_RAW - ($ - $$) db 0

SECTION .reloc start=PE_RELOC_FILE vstart=0x5000 align=4
reloc_start:
    dd 0x3000
    dd 12
