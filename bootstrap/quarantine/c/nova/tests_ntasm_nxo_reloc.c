...
static void expect_relocation(const NovaHostImage *image, const NtBuffer *file,
                              uint64_t index, const uint64_t expected[5]) {
  uint64_t out[10], runtime = 0; reset(out); cases++;
  uint64_t returned = invoke(image, file, file->size, index, out, 5, &runtime);
  CHECK(returned == 0, "relocation return"); CHECK(runtime == 0, "relocation runtime");
  for (unsigned i = 0; i < 5; i++) CHECK(out[i] == expected[i], "relocation word");
  for (unsigned i = 5; i < 10; i++) CHECK(out[i] == guard, "relocation guards");
}
...
int main(void) {
  const char *paths[] = {
    "toolchain/ntasm-nova/tests/nxo-reloc-entry.nova",
  ...
  if (image.memory) {
    ...
    if (file.bytes) {
      const uint64_t first[] = {3, 2, 0, 1, 0};
      expect_relocation(&image, &file, 0, first);
      expect_error(&image, &file, file.size, 1, 5, 811);
      expect_error(&image, &file, file.size, 0, 4, 809);
      expect_error(&image, &file, file.size - 1, 0, 5, 800);
      NtObject empty = object; empty.relocations = NULL; empty.relocation_count = 0;
      NtBuffer empty_file = {0}; NtDiagnostic empty_error = {0};
      CHECK(nt_object_write(&empty, &empty_file, &empty_error), "oracle writes empty-reloc NXO");
      if (empty_file.bytes) {
        expect_error(&image, &empty_file, empty_file.size, 0, 5, 811);
        free(empty_file.bytes);
      }
      free(file.bytes);
    }
  }
  ...
  printf("NTASM_NOVA_NXO_RELOC checks=%u failures=%u cases=%u\n", checks, failures, cases);
  return failures ? 1 : 0;
}
