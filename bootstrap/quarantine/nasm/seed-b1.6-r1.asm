...
; Quarantaine temporaire autorisée pour le tracer B1.6 uniquement.
; Commande de matérialisation quarantainée :
; nasm -f bin bootstrap/quarantine/nasm/seed-b1.6-r1.asm -o bootstrap/quarantine/nasm/out/seed-b1.6-r1.efi
...
entry:
    ...
    lea rdi, [rbp + 0x28]
    xor eax, eax
    mov ecx, 30                     ; zéro jusqu'aux slots temporaires B1.6
.zero_frame:
    mov [rdi], rax
    add rdi, 8
    dec ecx
...
semantic_eof:
    ...
    jae cleanup                     ; zéro constante est valide à B1.6
...
text_end:
    %if (text_end - text_start) > 0x1800
        %error "B1.6 .text depasse 0x1800"
    %endif
    times 0x1800 - ($ - $$) db 0
...
diagnostic_template:
    ...
    times 0x110 - ($ - $$) db 0
success_b16:
    ; NG_S0_T06_OK\r\n — succès temporaire du tracer B1.6.
    dw 'N','G','_','S','0','_','T','0','6','_','O','K',13,10,0
abi_kw:
    db 'abi'
efi_x64_kw:
    db 'efi_x64'
