...
; Quarantaine temporaire autorisée pour le tracer B1.7 uniquement.
; Commande de matérialisation quarantainée :
; nasm -f bin bootstrap/quarantine/nasm/seed-b1.7-r1.asm -o bootstrap/quarantine/nasm/out/seed-b1.7-r1.efi
...
entry:
    ...
    lea rdi, [rbp + 0x28]
    xor eax, eax
    mov ecx, 30                     ; zéro jusqu'aux slots temporaires B1.7
.zero_frame:
    mov [rdi], rax
    add rdi, 8
    dec ecx
...
semantic_after_entry:
    call classify_const_or_entry
    cmp eax, 1
    je semantic_e200_at_begin
    cmp eax, 2
    je semantic_e200_at_begin
    jmp semantic_success
...
text_end:
    %if (text_end - text_start) > 0x1A00
        %error "B1.7 .text depasse 0x1A00"
    %endif
    times 0x1A00 - ($ - $$) db 0

SECTION .rdata start=0x1E00 vstart=0x3000 align=16
rdata_start:
loaded_image_guid:
...
diagnostic_template:
    ...
    times 0x110 - ($ - $$) db 0
success_b17:
    ; NG_S0_T07_OK\r\n — succès temporaire du tracer B1.7.
    dw 'N','G','_','S','0','_','T','0','7','_','O','K',13,10,0
abi_kw:
    db 'abi'
efi_x64_kw:
    db 'efi_x64'
...
SECTION .data start=0x2200 vstart=0x4000 align=8
data_start:
    dq 0
data_end:
    times 0x200 - ($ - $$) db 0

SECTION .reloc start=0x2400 vstart=0x5000 align=4
reloc_start:
    dd 0x3000
    dd 12
