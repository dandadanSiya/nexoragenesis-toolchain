...
static void ok(const NovaHostImage*i,const char*n,const char*s,uint64_t f,uint64_t m,uint64_t b){Run r=run(i,s,3);cases++;checks++;if(r.returned||r.runtime||r.out[0]!=f||r.out[1]!=m||r.out[2]!=b){failures++;fprintf(stderr,"FAIL ok %s\n",n);}}
...
int main(void){const char*paths[]={"toolchain/ntasm-nova/tests/register-bindings-entry.nova","toolchain/ntasm-nova/register_bindings.nova","toolchain/ntasm-nova/register_widths.nova","toolchain/ntasm-nova/parameters.nova","toolchain/ntasm-nova/callable_names.nova","toolchain/ntasm-nova/semantics.nova","toolchain/ntasm-nova/parser.nova","toolchain/ntasm-nova/ast.nova","toolchain/ntasm-nova/lexer.nova"};char*sources[9]={0};NtFInput inputs[9];for(unsigned i=0;i<9;i++){size_t n=0;sources[i]=load(paths[i],&n);inputs[i]=(NtFInput){paths[i],sources[i],n};}int all=1;for(unsigned i=0;i<9;i++)if(!sources[i])all=0;CHECK(all,"sources load");NtArtifact a={0};int compiled=all&&nova_compile_many_v2(inputs,9,&a);CHECK(compiled,"modules compile");if(!compiled)fprintf(stderr,"E%u %zu:%zu %s\n",a.error.code,a.error.line,a.error.column,a.error.message);NovaHostImage image={0};CHECK(compiled&&nova_host_map(&a,&image),"image maps");
 if(image.memory){const char*distinct="module m\ntarget x86_64-nexora-none\nsection .text {\nfn f(in a:u64 @rcx,in b:u64 @rdx)->u64\neffects {}\nclobbers {} {\nreturn a\n}\n}\n";ok(&image,"distinct",distinct,1,0,2);const char*dup="module m\ntarget x86_64-nexora-none\nsection .text {\nfn f(in a:u64 @rcx,in b:u64 @rcx)->u64\neffects {}\nclobbers {} {\nreturn a\n}\n}\n";err(&image,"duplicate function register",dup,3,317,0);const char*dupimport="module m\ntarget x86_64-nexora-none\nimport a.f as f: fn(in a:u64 @rcx,in b:u64 @rcx)->u64\neffects {}\nclobbers {}\n";err(&image,"duplicate import register",dupimport,3,317,0);const char*none="module m\ntarget x86_64-nexora-none\nsection .text {\nfn f(in a:u64,in b:u64)->u64\neffects {}\nclobbers {} {\nreturn a\n}\n}\n";ok(&image,"without bindings",none,1,0,0);const char*separate="module m\ntarget x86_64-nexora-none\nsection .text {\nfn f(in a:u64 @rcx)->u64\neffects {}\nclobbers {} {\nreturn a\n}\nfn g(in b:u64 @rcx)->u64\neffects {}\nclobbers {} {\nreturn b\n}\n}\n";ok(&image,"separate declarations",separate,2,0,2);const char*families[][2]={{"rax","eax"},{"rcx","ecx"},{"rdx","edx"},{"rbx","ebx"},{"rsp","esp"},{"rbp","ebp"},{"rsi","esi"},{"rdi","edi"},{"r8","r8d"},{"r9","r9b"},{"r10","r10d"},{"r11","r11w"},{"r12","r12b"},{"r13","r13d"},{"r14","r14w"},{"r15","r15b"}};for(unsigned k=0;k<16;k++){char alias[512];snprintf(alias,sizeof(alias),"module m\ntarget x86_64-nexora-none\nsection .text {\nfn f(in a:u64 @%s,in b:u64 @%s)->u64\neffects {}\nclobbers {} {\nreturn a\n}\n}\n",families[k][0],families[k][1]);err(&image,families[k][0],alias,3,317,0);}err(&image,"short",distinct,2,800,1);err(&image,"invalid","target x86_64-nexora-none\n",3,800,1);}
 if(image.memory) nova_host_unmap(&image);
 if(compiled) nt_artifact_free(&a);
 for(unsigned i=0;i<9;i++) free(sources[i]);
 printf("NTASM_NOVA_REGISTER_BINDINGS checks=%u failures=%u cases=%u\n",checks,failures,cases);
 return failures?1:0;
}
