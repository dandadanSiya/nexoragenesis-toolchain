 ...
 * Installs a copied IDT with only vector13 redirected to an NTASM-produced
 * same-CPL/hardware-error raw stub. No inline assembly or foreign assembler source.
 */
#include <stddef.h>
#include <stdint.h>
...
/* Select a reserved MSR, save flags in R8, execute WRMSR to raise #GP(0),
 * capture flags after IRETQ in RAX and return XOR. */
static const uint8_t CONTROL gp_code[] = {
...
STATUS EFIAPI efi_main(HANDLE self, SYSTEM *system) {
  ...
  if (ERROR(status) || !uart)
    return EFI_ERROR(3);
  serial("BOOTSTRAP_C_TEST A6_GP_IRETQ\r\n");
  ...
  if (!original.base || table_size < 14 * sizeof(GATE) ||
  ...
  make_gate(&((GATE *)idt_copy)[13], handler, selector);
  ...
  U64 flags_delta = ((READ_FN)(uintptr_t)gp_code)();
...
finish:
  ...
  if (cleanup_bad && !error)
    error = 20;
  if (error) {
    serial("NG_A6_GP_IRETQ_BAD code=");
    hex(error);
    serial(" status=");
    hex(status);
    serial("\r\n");
