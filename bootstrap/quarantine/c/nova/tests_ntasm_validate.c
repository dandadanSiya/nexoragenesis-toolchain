...
static void expect(const NovaHostImage*i,const char*n,const char*s,uint64_t words,uint64_t code,int untouched){Run r=run(i,s,words);cases++;checks++;if(r.returned!=code||r.runtime||(code==0&&(r.out[0]!=13||r.out[1]==0||r.out[2]==0||r.out[3]!=0))||(untouched&&(r.out[0]!=guard||r.out[1]!=guard))){failures++;fprintf(stderr,"FAIL %s ret=%llu runtime=%llu out=%llu,%llu,%llu,%llu\n",n,(unsigned long long)r.returned,(unsigned long long)r.runtime,(unsigned long long)r.out[0],(unsigned long long)r.out[1],(unsigned long long)r.out[2],(unsigned long long)r.out[3]);}}
int main(void){const char*paths[]={"toolchain/ntasm-nova/tests/validate-entry.nova","toolchain/ntasm-nova/validate.nova","toolchain/ntasm-nova/calls.nova","toolchain/ntasm-nova/refs.nova","toolchain/ntasm-nova/clobbers.nova","toolchain/ntasm-nova/contracts.nova","toolchain/ntasm-nova/effects.nova","toolchain/ntasm-nova/register_widths.nova","toolchain/ntasm-nova/register_bindings.nova","toolchain/ntasm-nova/bindings.nova","toolchain/ntasm-nova/locals.nova","toolchain/ntasm-nova/parameters.nova","toolchain/ntasm-nova/symbol_names.nova","toolchain/ntasm-nova/callable_names.nova","toolchain/ntasm-nova/semantics.nova","toolchain/ntasm-nova/parser.nova","toolchain/ntasm-nova/ast.nova","toolchain/ntasm-nova/lexer.nova"};char*sources[18]={0};NtFInput inputs[18];for(unsigned i=0;i<18;i++){size_t n=0;sources[i]=load(paths[i],&n);inputs[i]=(NtFInput){paths[i],sources[i],n};}int all=1;for(unsigned i=0;i<18;i++)if(!sources[i])all=0;CHECK(all,"sources load");NtArtifact a={0};int compiled=all&&nova_compile_many_v2(inputs,18,&a);CHECK(compiled,"modules compile");if(!compiled)fprintf(stderr,"E%u %zu:%zu %s\n",a.error.code,a.error.line,a.error.column,a.error.message);NovaHostImage image={0};CHECK(compiled&&nova_host_map(&a,&image),"image maps");
 if(image.memory){
 ...
 for(unsigned i=0;i<18;i++) free(sources[i]);
