...
; Quarantaine temporaire autorisée pour le tracer B1.8 uniquement.
; Commande de matérialisation quarantainée :
; nasm -f bin bootstrap/quarantine/nasm/seed-b1.8-r1.asm -o bootstrap/quarantine/nasm/out/seed-b1.8-r1.efi
...
semantic_in_text:                 ; Classer seulement le premier identifiant du corps.
...
semantic_eof:
    cmp qword [rbp + F_SEM_STAGE], IN_TEXT ; EOF après .text est le succès provisoire T08.
    je semantic_success
...
.char:
    ...
    jmp .scan
.find:
    lea r9, [rel b18_body_words]   ; Table bornée terminée par une longueur nulle.
.entry:
...
validate_entry_descriptor:
    cmp qword [rbp + F_SEM_STAGE], IN_TEXT ; Le succès exige aussi l'en-tête texte.
    jne .bad
...
success_b18:
    ...
    dw 'N','G','_','S','0','_','T','0','8','_','O','K',13,10,0
abi_kw:
    db 'abi'
efi_x64_kw:
    db 'efi_x64'
...
SECTION .data start=0x2800 vstart=0x4000 align=8
data_start:
    dq 0
data_end:
    times 0x200 - ($ - $$) db 0

SECTION .reloc start=0x2A00 vstart=0x5000 align=4
reloc_start:
    dd 0x3000
    dd 12
